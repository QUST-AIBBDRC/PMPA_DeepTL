...
for train, test in skf.split(X,y): 
    ...
    print('DT:acc=%f,precision=%f,npv=%f,sensitivity=%f,specificity=%f,mcc=%f,f1=%f,roc_auc=%f'
...
yscore_sum.to_csv('yscore_sum_NB_Near_Miss_train_fusion.csv')
ytest=ytest[np.array(range(1,row)),:]
ytest_sum = pd.DataFrame(data=ytest)
ytest_sum.to_csv('ytest_sum_NB_Near_Miss_train_fusion.csv')
fpr, tpr, _ = roc_curve(ytest[:,0], yscore[:,0])
auc_score=np.mean(scores, axis=0)[7]
lw=2
plt.plot(fpr, tpr, color='darkorange',
lw=lw, label='DT ROC (area = %0.2f%%)' % auc_score)
...
data_csv.to_csv('NB_Near_Miss_train_fusion.csv')
